...
async function clearCloudInventory() {
  ...
  const ok = window.confirm("确定清空当前账号的云端库存吗？这不会删除公共书目信息。");
  if (!ok) return;
...
function renderShelves() {
  ...
  els.activeShelfMeta.textContent = activeShelf ? `${booksOnShelf(activeShelf.name).length} 种书在这个书架上。扫码识别后可放入或移出。` : "选择一个书架后可查看其中图书。";
  ...
  if (!activeShelf) {
    ...
    if (!shelfBooks.length) {
      ...
      shelfBooks.forEach((book) => {
        ...
        remove.className = "small-button minus";
        remove.type = "button";
        remove.textContent = "移";
        remove.addEventListener("click", () => assignBookToShelf(book, ""));
...
function updatePendingActionUi() {
  ...
  els.pendingBookMeta.textContent = `${book.isbn} · 当前库存 ${Number(existing?.count || 0)} 本`;
