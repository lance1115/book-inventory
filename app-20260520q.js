...
function setStorageNote(lines) {
  els.storageNote.replaceChildren(
    ...lines.map((line) => {
...
function bindEvents() {
  ...
  els.manualForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      els.manualDelta.value = "0";
      [els.manualCategory, els.manualShelf, els.manualPrice, els.manualNote].filter(Boolean).forEach((input) => {
        input.value = "";
      });
