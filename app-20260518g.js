...
async function pushLocalToCloud() {
  ...
  try {
    ...
    if (books.length) {
      const { error: booksError } = await state.supabase.from("books").upsert(books.map(cloudBookPayload), {
        onConflict: "isbn",
      });
      if (booksError) throw booksError;

      const { error: inventoryError } = await state.supabase
        .from("inventory")
        .upsert(books.map(cloudInventoryPayload), { onConflict: "user_id,isbn" });
      if (inventoryError) throw inventoryError;
    }
...
async function saveBookToCloud(book, effectiveDelta, reason) {
  if (!state.supabase || !state.user) return;
  const payload = cloudBookPayload(book);
  const inventory = cloudInventoryPayload(book);
  const { error: bookError } = await state.supabase.from("books").upsert(payload, { onConflict: "isbn" });
  if (bookError) throw bookError;

  const { error: inventoryError } = await state.supabase.from("inventory").upsert(inventory, { onConflict: "user_id,isbn" });
  if (inventoryError) throw inventoryError;
...
async function lookupCachedBook(isbn) {
  ...
  const { data, error } = await state.supabase.from("lookup_cache").select("title,authors,publisher,source").eq("isbn", isbn).maybeSingle();
...
async function handleDetectedCode(rawCode, reason = "扫码") {
  ...
  setStatus(`已扫到 ${isbn}，已停止扫码，正在自动查询书名...`);
  ...
  try {
    ...
    if (state.mode === "lookup") {
      ...
      pulseSuccess();
      if (stockedBook) {
        setStatus(`${getBookTitle(stockedBook)}：当前库存 ${stockedBook.count} 本。扫下一本请再点“开始扫码”。`, "success");
      } else if (book.title) {
        setStatus(`${getBookTitle(book)}：库存中暂无记录。扫下一本请再点“开始扫码”。`, "success");
      } else {
    ...
    if (delta < 0 && currentCount <= 0) {
      pulseSuccess();
      setStatus(`${getBookTitle(book)} 当前库存为 0，不能继续出库。扫下一本请再点“开始扫码”。`, "error");
      return;
    }
    ...
    setStatus(`${getBookTitle(updated)} ${delta > 0 ? "入库" : "出库"}成功，当前库存 ${updated.count} 本。扫下一本请再点“开始扫码”。`, "success");
  } catch (error) {
...
async function startScanner() {
  ...
  try {
    ...
    setStatus("摄像头已开启。识别到一本后会自动停止；扫下一本请再点“开始扫码”。");
  } catch (error) {
