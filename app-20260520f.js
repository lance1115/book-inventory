...
function updateCloudUi() {
  ...
  els.clearAll.disabled = !signedIn || state.syncBusy || state.role !== "admin";
...
async function deleteShelf(shelf) {
  ...
  const ok = window.confirm(`确定删除书架“${shelf.name}”吗？${shelfBooks.length ? "书架里的图书会保留库存，只移出这个书架。" : "这个书架没有图书。"}`);
  if (!ok) return;
...
function upsertBook(book, delta, reason, options = {}) {
  ...
  if (state.role === "staff" && !["扫码", "照片识别", "确认入库", "确认出库"].includes(reason)) {
...
function bindEvents() {
  ...
  els.manualForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      els.manualDelta.value = "0";
      [els.manualCategory, els.manualShelf, els.manualLocation, els.manualCoverUrl, els.manualPrice, els.manualPublishedDate, els.manualEdition, els.manualNote].filter(Boolean).forEach((input) => {
        input.value = "";
      });
  ...
  els.clearAll.addEventListener("click", clearCloudInventory);
