...
async function clearCloudInventory() {
  ...
  const ok = window.confirm("确定清空当前账号的云端库存吗？这不会删除公共书目信息，也不会自动清空本机库存。");
  if (!ok) return;
  ...
  try {
    ...
    setCloudStatus("云端库存已清空。本机库存如需清空，请点库存检索里的“清空”。", "success");
  } catch (error) {
...
function changeInventory(isbn, delta, reason = "扫码") {
  ...
  setStatus(`${getBookTitle(updated)}：库存 ${updated.count} 本${state.user ? "，正在同步云端。" : ""}`, "success");
  return updated;
}
